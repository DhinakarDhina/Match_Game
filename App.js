...
const App = () => {
  ...
  const [activeTabId, setActiveTabId] = useState('FRUIT')
  const [matchImageUrl, setMatchImageUrl] = useState(imagesList[0].imageUrl)
  ...
  const onClickThumbnail = id => {
    const clicked = imagesList.find(each => each.id === id)
    if (clicked.imageUrl === matchImageUrl) {
      setScore(prev => prev + 1)
      let newImage = matchImageUrl
      while (newImage === matchImageUrl) {
        const index = Math.floor(Math.random() * imagesList.length)
        newImage = imagesList[index].imageUrl
      }
      setMatchImageUrl(newImage)
    } else {
      setIsGameOver(true)
    }
  }
  ...
  const onPlayAgain = () => {
    setScore(0)
    setTime(60)
    setActiveTabId('FRUIT')
    setMatchImageUrl(imagesList[0].imageUrl)
    setIsGameOver(false)
  }
  ...
  return (
    <div>
      ...
      {isGameOver ? (
        <ScoreCard score={score} onPlayAgain={onPlayAgain} />
      ) : (
        <div>
          <img src={matchImageUrl} alt="match" />
          <ul>
            {tabsList.map(tab => (
              <TabItem
                key={tab.tabId}
